...
const { C, M } = unstringifyBigInts(require("../node_modules/circomlib/src/poseidon_constants.json"))
...
const genRandomNonce = (): BigInt => {
    ...
    const privKey: PrivKey = rand % max
    assert(privKey < max)
...
const poseidonEncrypt = (
    ...
    let state = [
        ...
        sharedKey[0],
        sharedKey[1],
        BigInt(nonce) + BigInt(msg.length) * two128,
    ]
    ...
    for (let i = 0; i < cipherLength / 3; i ++) {
        ...
        state[1] = (BigInt(state[1]) + BigInt(message[i * 3])) % SNARK_FIELD_SIZE
        state[2] = (BigInt(state[2]) + BigInt(message[i * 3 + 1])) % SNARK_FIELD_SIZE
        state[3] = (BigInt(state[3]) + BigInt(message[i * 3 + 2])) % SNARK_FIELD_SIZE
...
const poseidonDecrypt = (
    ...
    let state = [
        ...
        sharedKey[0],
        sharedKey[1],
        BigInt(nonce) + BigInt(length) * two128,
    ]
    ...
    for (let i = 0; i < n; i ++) {
        ...
        message.push((BigInt(ciphertext[i * 3]) - BigInt(state[1])) % SNARK_FIELD_SIZE)
        message.push((BigInt(ciphertext[i * 3 + 1]) - BigInt(state[2])) % SNARK_FIELD_SIZE)
        message.push((BigInt(ciphertext[i * 3 + 2]) - BigInt(state[3])) % SNARK_FIELD_SIZE)
    ...
    // Check if the last (3 - (l mod 3)) elements of the message are 0
    if (length > 3) {
        if (length % 3 === 2) {
            assert(BigInt(message[message.length - 1]) === BigInt(0))
        } else if (length % 3 === 1) {
            assert(BigInt(message[message.length - 1]) === BigInt(0))
            assert(BigInt(message[message.length - 2]) === BigInt(0))
        }
    }
    ...
    assert(BigInt(ciphertext[ciphertext.length - 1]) === BigInt(state[1]))
